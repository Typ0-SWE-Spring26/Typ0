...
class StartMenu:
    ...
    async def run(self):
        ...
        while self.running:
            ...
            self.button_rect = pygame.Rect(self.screen.get_width() // 4, self.screen.get_height() // 2+210, 400, 75)
            pygame.draw.rect(self.screen, (40, 80, 40), self.button_rect, border_radius=8)
            instruction_text = self.font_small.render("Press M for Multiplayer", True, (200, 255, 200))
            self.screen.blit(instruction_text, instruction_text.get_rect(
